...
# --------------获取数据集-----------
f = open('庆余年220.csv', encoding='gb18030')
...
# 扩展前的词典
df = pd.read_excel('中文情感词汇本体NAU.xlsx')
print(df.head(10))
...
# stop_list = pd.read_csv('hit_stopwords.txt',
#                         engine='python',
#                         encoding='utf-8',
#                         delimiter="\\n",
#                         names=['t'])
...
def emotion_caculate(text):
    ...
    good_list = []
    happy_list = []
    wordlist = txt_cut(text)  # 调用分词函数
    wordset = set(wordlist)  # 转为集合
    # wordfreq = []
    for word in wordset:
    ...
    indexs = ['length', 'positive', 'negative',
              'anger', 'disgust', 'fear',
              'sadness', 'surprise', 'good',
              'happy']
